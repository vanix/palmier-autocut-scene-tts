...
| Phase | 內容 |
|:-----:|:------|
| 1. 問卷 | 素材路徑、跳過檔案、場景秒數、旁白風格、TTS 語音 |
...
Phase 5 ─ 輸出 palmier_script.json + subtitles.srt + script_narrative.txt
  │
  ▼
匯入 Palmier Pro → add_clips 上片 → add_texts 上字幕 → import_media TTS → add_clips 旁白音軌 → 輸出成品
```
...
```
素材資料夾/
├── palmier_script.json        ← → 給 Palmier Pro MCP（含 tts_file）
├── subtitles.srt               ← → 字幕檔
├── script_narrative.txt       ← → 給人看的腳本
└── temp/                      ← 暫存檔（可清空）
    ├── manifest.json          ← 場景清單（含描述、旁白、TTS）
    ├── scripts/               ← Python 腳本
    │   ├── run_scene_detect.py
    │   ├── qwen_describe.py
    │   ├── generate_tts.py
